...
| 컨텍스트 파일 | `template/…/{CLAUDE,AGENTS}.md` | 루트 `CLAUDE.md`·`AGENTS.md` | init-once (`_skip_if_exists`) |
...
| **init-once** (`_skip_if_exists`) | `CLAUDE.md`·`AGENTS.md`·`commit-scopes.md` | 최초 배포 시 1회만 생성, 이후 로컬 수정 보존(update가 덮어쓰지 않음) |
...
|------------|------------------------------|------------------------|
| **E1-S1** (Copier 골격) | §3 매핑 + §5 배포 방식 | `template/` source 트리 골격 + `copier.yml` 작성, `_skip_if_exists`(init-once) 배선 |
